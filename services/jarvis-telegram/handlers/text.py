...
"""
Text message handler — routes text to Claude CLI or Gemini depending on context.
"""
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Process a text message — uses Claude CLI bridge if available, else Gemini."""
    ...
    # Route: Claude CLI if available, else Gemini with context
    if claude_available:
        ...
        response = await gemini.model.generate_content(context_parts)
        response = response.text
    else:
        response = "No AI backend configured. Set GEMINI_API_KEY or install Claude CLI."
